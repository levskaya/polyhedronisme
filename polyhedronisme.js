(function() {
  ...
  vertIndex = function(vertlist, vert) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      _ref2 = _ref[_i], i = _ref2[0], v = _ref2[1];
      if (mag(sub(vert, v)) < 1E-6) return i;
    }
    return -1;
  };
  ...
  uniquifyverts = function(vertlist) {
    ...
    for (_i = 0, _len = vertlist.length; _i < _len; _i++) {
      ...
      for (_j = 0, _len2 = uniqverts.length; _j < _len2; _j++) {
        u = uniqverts[_j];
        if (mag(sub(v, u)) < 1E-6) already_present = true;
      }
      if (!already_present) uniqverts.push(v);
    }
    return uniqverts;
  };
  ...
  meshFix = function(poly) {
    var edge0, edge1, f, faces, lengthAlong, newf, newfaces, newpoly, newv, testv, tmpv, v, verts, vno, _i, _j, _k, _l, _len, _len2, _len3, _len4, _ref, _ref2;
    faces = poly.face;
    verts = poly.xyz;
    lengthAlong = function(fromV, toV, midV) {
      return dot(unit(sub(toV, fromV)), sub(midV, fromV));
    };
    newfaces = [];
    for (_i = 0, _len = faces.length; _i < _len; _i++) {
      f = faces[_i];
      ...
      for (_j = 0, _len2 = f.length; _j < _len2; _j++) {
        ...
        for (_k = 0, _len3 = _ref.length; _k < _len3; _k++) {
          ...
          if (edge_vert_isect(edge0, edge1, testv)) tmpv.push(vno);
        }
        tmpv.sort(function(a, b) {
          return lengthAlong(edge0, edge1, verts[a]) - lengthAlong(edge0, edge1, verts[b]);
        });
        for (_l = 0, _len4 = tmpv.length; _l < _len4; _l++) {
          newv = tmpv[_l];
          newf.push(newv);
        }
        newf.push(v);
  ...
  simpleFaceCheck = function(faces) {
    var alledges, e, e2, i, new_face, newedges, nuked, _i, _len, _ref, _ref2;
  ...
  uniteFaces = function(poly) {
    var connected_set, dpoly, e, edict, face_idx, face_set, face_sets, faces_to_join, hash, idx, isNeighbor, j, joinset, joinsets, k, ndict, newfaces, newpoly, normals, normhash, splitidx, t, v, _i, _j, _k, _l, _len, _len2, _len3, _len4, _len5, _len6, _m, _n, _o, _ref, _ref2, _ref3, _ref4, _ref5, _ref6, _results;
    normals = poly.normals();
    ...
    for (_j = 0, _len = face_idx.length; _j < _len; _j++) {
      idx = face_idx[_j];
    ...
    for (_m = 0, _len4 = face_sets.length; _m < _len4; _m++) {
      ...
      while (face_set.length > 0) {
        ...
        for (j = 0, _ref4 = face_set.length - 1; 0 <= _ref4 ? j <= _ref4 : j >= _ref4; 0 <= _ref4 ? j++ : j--) {
          if (__indexOf.call(connected_set, j) >= 0) continue;
        ...
        connected_set = [];
        if (simpleFaceCheck(_.map(joinset, function(i) {
          return poly.face[i];
        }))) {
          joinsets.push(joinset);
        } else {
          for (splitidx = 0, _ref6 = joinset.length - 1; 0 <= _ref6 ? splitidx <= _ref6 : splitidx >= _ref6; 0 <= _ref6 ? splitidx++ : splitidx--) {
            if (simpleFaceCheck(_.map(joinset.slice(0, splitidx + 1 || 9e9), function(i) {
              return poly.face[i];
            })) && simpleFaceCheck(_.map(joinset.slice(splitidx), function(i) {
              return poly.face[i];
            }))) {
              break;
            }
          }
          joinsets.push(joinset.slice(0, splitidx + 1 || 9e9));
          joinsets.push(joinset.slice(splitidx + 1));
        }
      }
    }
  ...
  $(function() {
    var specs, t0, t1, t2, ttmp, urlParams, v, _i, _len, _ref;
    init();
    urlParams = parseurl();
