...
// works pretty well, but occasionally infinite-loops and hangs tab :(
// import * as martinez from 'martinez-polygon-clipping';
// try adding this loop-guard PR by hand: https://github.com/w8r/martinez/pull/66/files

// best polygon clipping lib:  lots of bugfixes in, almost always accurate:
...
const simple_intersect = function (A_Vs2d, B_Vs2d) {
  ...
  try {
    ...
    const result = polygonclipping.intersection([A_Vs2d_], [B_Vs2d_]);
    ...
    console.log(e);
    // console.log(printpoly(A_Vs2d_), printpoly(B_Vs2d_));
    console.log(printpoly(A_Vs2d), printpoly(B_Vs2d));
    return [];
  }
}
window.simple_intersect=simple_intersect;
/*
...
*/
export const get_inset_verts = function (verts, alpha) {
  let c = calcCentroid(verts);
  return verts.map(v => tween(v, c, alpha))
}
export const get_inset_verts2d = function (verts, alpha) {
  let c = calcCentroid2d(verts);
  return verts.map(v => tween2d(v, c, alpha))
}

export const get_extents = function (verts) {
...
export const get_2dextents = function (verts) {
  let xs = verts.map(x => x[0]);
  let ys = verts.map(x => x[1]);
  return [
    [Math.min.apply(null, xs), Math.max.apply(null, xs)],
...
export const get_2d_vertices = function (verts) {
...
export const project_screen_ray_onto_plane = function (sigma, z0, screen_coord, plane_c, plane_n) {
...
export const P_obscures_Q = function (P, Q) {
  ...
  // console.log('intersect ', printpoly(Pvs2d), printpoly(Qvs2d));
...
export const sortFacesCarefully = function (poly) {
  ...
  // Guard against NaNs... (TODO: fix the upstream cause)
  if (_.reduce(_.map(_.flatten(faceverts2d), x => isNaN(x)), (x, y) => x || y)) {
    console.log(faceverts2d);
  ...
  try {
    ...
    console.log(e);
    // if topological sort fails, just preserve z-centroid order.
    for (let vidx of visible_faces) {
      zsortIndex.push(original_face_indices[vidx])
    }
  }
